...
Function Create-MonitoringEntity {
   ...
   try {
      $entityIds = (((Invoke-RestMethod -Uri $entityuri -Method GET -Headers $authToken).values).metadata).instanceID
   }
   catch {
   ...
   foreach($server in $envServers) {
      if($entityIds -notcontains $server.id) {
         ...
         $body = @{'label'=$server.name; 'ip_addresses'=$ipobject;'agent_id'=$agentID;'metadata'=@{'serverURI'=$serveruri;'instanceID'=$server.id}} | ConvertTo-Json -Depth 3
         try {
         ...
         foreach($env in $envs) {
            ...
            foreach($server in $envServers) {
               if($agent_tokens.label -notcontains $server.id) {
                  $body = @{'label' = $server.id} | ConvertTo-Json
                  try {
                     $agentToken = (Invoke-RestMethod -Uri $tokenuri -Method POST -Headers $authToken -Body $body -ContentType application/json).token
                  }
                  catch {
                     Write-EventLog -LogName DevOps -Source RS_rsCloudServersOpenStack -EntryType Error -EventId 1002 -Message "Failed to create Agent Token `n $tokenuri `n $($_.Exception.Message)"
                  }
               }
               else {
               ...
               try {
                  ...
                  powershell.exe (Join-Path $scriptData.Directory.scriptsRoot -ChildPath ClientDSC.ps1) -Node $server.name, -ObjectGuid $server.id, -MonitoringID $server.id, -MonitoringToken $agentToken
      ...
      foreach($server in $envServers) {
         if($currentMofs -notcontains $server.id) {
            try {
               ...
               Write-EventLog -LogName DevOps -Source RS_rsCloudServersOpenStack -EntryType Information -EventId 1000 -Message "Creating MOF file for server $server.name $server.id `n $currentMofs `n $server.id"
               Remove-Item ("C:\Program Files\WindowsPowerShell\DscService\Configuration\" + $server.id + "*") -Force
               powershell.exe (Join-Path $scriptData.Directory.scriptsRoot -ChildPath ClientDSC.ps1) -Node $server.name, -ObjectGuid $server.id, -MonitoringID $server.id, -MonitoringToken $agentToken
            }
            catch {
               Write-EventLog -LogName DevOps -Source RS_rsCloudServersOpenStack -EntryType Error -EventId 1002 -Message "Failed to create configuration .MOF file for $server.name `n $($_.Exception.Message)"
            }
         }
      }
   }
